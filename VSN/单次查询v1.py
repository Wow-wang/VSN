...
def keygen():
    sk_du = random.randint(1,10)# sk_du随机整数
    pk_du = P * sk_du
    return sk_du,pk_du
...
def update(pk_du,op,w,ind,w_st_c):
    ...
    if w_st_c.get(w) == None:
        st_0 = random.randint(1,10)# lamda随机生成字符串
        c = 0
    else:
        st_0,c=w_st_c[w]
    
    #对称加密
    k = hmac.new(b'yangxu').digest()
    model = AES.MODE_ECB
    tok_server_addr = (str(st_0)).zfill(16)
    t_addr = tok_server_addr.encode('utf-8')
    aes = AES.new(k, model)
    st_1 = aes.encrypt(t_addr).zfill(32)
    # print("st_1",st_1)
    w_st_c[w] = st_1,c+1
...
def decrypt_mei(sk_du,w,mei):
    ...
    opind = bytes(a ^ b for a, b in zip(temp,mei))
    return opind
...
store_var_contract = w3.eth.contract(
    address=w3.toChecksumAddress('0xf8C55663a726fb5aA81fE48b0F26A0dc9e610e64'),
   abi=abi_build_index)

sk_du, pk_du = keygen()
update(pk_du,"op","w","ind",w_st_c)
Tw = trapdoor(sk_du,"w")
