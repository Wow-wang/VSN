...
def update(Kw_File_Use,G_o):
	for w in Kw_File_Use.keys():
		...
		# print(Thex.encode())
		t_lable = Web3.keccak(hexstr=Thex).hex
		t_lable = t_lable.encode('utf-8')
		print('length of t_lable:' + str(len(t_lable)))
		...
		for ind in Kw_File_Use[w]:
			...
			v = bytes(a ^ b for a,b in zip(l,(str(st_1)+"and"+ind).ljust(len(l),"0").encode()))#异或多少位？用什么加密方法
		...
		store_var_contract.functions.set_I(t_lable,P).transact({#32 32
			"from": from_account,
			"gas": 3000000,
			"gasPrice": 0,
		})

def update_one_entry(Kw_File_Use,G_o):
	for w in Kw_File_Use.keys():
		...
		print(type(Thex))
		####################################################################
		t_lable = Web3.keccak(hexstr=Thex)


		# t_lable = Web3.keccak(hexstr=Thex).hex()
		# print('length of t_lable:' + str(len(t_lable)))
		print('-----------python 算 t <--- H(T)-------------')
		print(t_lable)
		t = t_lable#测时间专用
		P = bytes(a ^ b for a,b in zip(t,(str(st_1)).zfill(len(t)).encode()))
		####################################################################
...
store_var_contract = w3.eth.contract(
	address=w3.toChecksumAddress('0xB88cAb2fCd7f708c95f4F93883c9fb68Eb697F18'),
	abi=abi_build_index)
